...
export default function DestinationDetails() {
  ...
  Promise.allSettled([
    ...
    api.get("/packages", { params: { size: 6 } }),
    api.get("/reviews", { params: { destinationId: id, status: "APPROVED", size: 6 } }),
  ])
    .then(([dRes, pRes, rRes]) => {
      if (dRes.status === "rejected") {
        setError(dRes.reason?.response?.data?.message || "Destination not found");
        return;
      }
      setD(dRes.value.data.data);

      const allPackages = pRes.status === "fulfilled"
        ? pRes.value.data.data.content || []
        : [];
      setPackages(allPackages.filter((p) => p.destinationId === Number(id)));

      setReviews(rRes.status === "fulfilled" ? rRes.value.data.data.content || [] : []);
    })
  ...
  return (
    <div>
      <div className="relative h-80 md:h-96 bg-cover bg-center" style={{ backgroundImage: `url('${d.imageUrl}')` }}>
      ...
      <div className="container-page py-10 grid lg:grid-cols-3 gap-8">
          ...
          {activities.length > 0 && (
            <section>
              ...
              <div className="flex flex-wrap gap-2">
                {activities.map((a) => (
                  <span key={a} className="px-3 py-1.5 bg-primary/10 text-primary rounded-full text-sm">
                    {a}
                  </span>
                ))}
              </div>
        ...
        <aside className="space-y-4">
          <div className="card p-6">
            ...
            <p className="text-2xl font-bold text-primary mt-1">{formatCurrency(d.estimatedCost)}</p>
            <p className="text-sm text-gray-500 mt-3">Best Time to Visit</p>
            <p className="font-medium mt-1">{d.bestTimeToVisit}</p>
            <Link to="/packages" className="btn-primary w-full mt-4">Browse Packages</Link>
          </div>
        </aside>
      </div>
    </div>
